...
- **自动种植** — 收获/铲除后自动购买种子并种植（支持自动最优策略或手动选择）
...
本工具需要小程序的登录凭证（code）才能连接服务器。code 为一次性使用，每次连接需重新获取。
...
> **注意**：code 为一次性凭证，使用后即失效，需重新进入小程序获取新的 code。
## 使用方式
